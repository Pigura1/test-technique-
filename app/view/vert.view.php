<h2>Ici vous trouverez les espaces verts dans paris</h2>

<?php

$string = file_get_contents("ilots-de-fraicheur-espaces-verts-frais.json");//lit et decode le contenu du fichier json en string 
    ...
    <?php if (is_array($json_a)) : ?>
      <?php foreach ($json_a as $item): ?>
        <tr>
          ...
          <td><?= htmlspecialchars($item['categorie'] ?? 'Information non disponible') ?></td>
        </tr>
      <?php endforeach; ?>
    <?php else : ?>
      <tr><td colspan="6">Erreur de lecture du fichier JSON.</td></tr><!--code d'erreur en cas de problème avec le json -->
    <?php endif; ?>
  </tbody>
</table>
